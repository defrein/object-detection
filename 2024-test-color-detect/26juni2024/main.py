...
def main():
    ...
    # Sesuaikan pengaturan kamera jika memungkinkan
    # cap.set(cv2.CAP_PROP_EXPOSURE, -4)  # Ubah sesuai dengan kebutuhan
    # cap.set(cv2.CAP_PROP_BRIGHTNESS, 0.7)  # Ubah sesuai dengan kebutuhan
...
def capture_frame(cap):
    ...
    # Adjust the brightness and contrast here
    # frame = cv2.convertScaleAbs(frame, alpha=1.0, beta=50)  # Increase brightness
...
def draw_rectangle(frame, start_x, start_y, end_x, end_y):
    ...
    cv2.rectangle(frame, (width//2 - box_size//2, height//2), (width//2 + box_size//2, height), (0, 255, 0), 2)
    return frame
    ...
    if stop_detect == 0:
        ...
        if color_detected:
            for coord, color in zip(coordinates, color_type):
                ...
                if color == MERAH:
                    if (coord[0] >= width//2 - box_size//2 and coord[0] <= width//2 + box_size//2) and coord[1] <= height//2:
                        status = MENCARI
                        aksi = MAJU_SEDANG
                        print("Objek berada di atas")
                    elif coord[0] <= width//2 - box_size//2 and coord <= height//2:
                        status = MENCARI
                    ...
                    elif (coord[0] >= width//2 - box_size//2 and coord[0] <= width//2 + box_size//2) and coord[1] > height//2:
                        status = DAPAT_MERAH
...
def detect_color_target(frame, start_x, start_y):
        ...
        if area > max_area_red:
            ...
            if radius > 10:
                ...
                if M["m00"] != 0:
                    ...
                    # Ignore red objects in the specified areas
                    if not ((center[0] >= width // 2 + box_size // 2 and center[1] >= height // 2) or
                            (center[0] <= width // 2 - box_size // 2 and center[1] >= height // 2)):
                        max_area_red = area
                        max_contour_red = c
    ...
    for c in cnts_blue:
        ...
        if radius > 30:
            ...
            if M["m00"] != 0:
                center = (int(M["m10"] / M["m00"]), int(M["m01"] / M["m00"]))
                if (center[0] >= width//2 - box_size//2 and center[0] <= width//2 + box_size//2) and center[1] > height//2:
                    detected_coordinates.append(center)
                    color_type.append(BIRU)
                    cv2.circle(frame, (int(x), int(y)), int(radius), (0, 255, 255), 2)
                    cv2.circle(frame, (center[0], center[1]), 5, (0, 0, 255), -1)
...
if __name__ == '__main__':
    ...
    colorLowerRed = np.array([0, 120, 70])
    colorUpperRed = np.array([10, 255, 255])
    colorLowerRed2 = np.array([170, 120, 70])
    colorUpperRed2 = np.array([180, 255, 255])
