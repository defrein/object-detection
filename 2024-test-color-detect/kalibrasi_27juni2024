...
def receive_from_arduino():
    ...
    global colorLowerRed, colorUpperRed, colorLowerRed2, colorUpperRed2
    global warnaBiruLower, warnaBiruUpper
    sensor_atas = ''
    while True:
        if ser.in_waiting > 0:
            ...
            if data == "ZONA_MERAH":
                pass
            if data == "ZONA_BIRU":
                colorLowerRed = colorLowerBlue
                colorUpperRed = colorUpperBlue
                colorLowerRed2 = colorLowerBlue
                colorUpperRed2 = colorUpperBlue
...
if __name__ == "__main__":
    ...
    warnaBiruLower = np.array([102, 164, 66])
    warnaBiruUpper = np.array([135, 255, 255])
